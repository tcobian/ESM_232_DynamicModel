...
forest<- function(time, initial_kgC, parms){
  ...
  if(temp < 0) {kgC = 0}
  else if(kgC < canopy_thresh) {kgC * early_growth}
  else if(kgC > canopy_thresh) {kgC * later_growth}
  else(kgC >= K) {kgC = K}
  
  
    #kgC = ifelse(temp < 0, 0, kgC)
    #kgC = ifelse(kgC < canopy_thresh, kgC * early_growth, kgC)
    #kgC = ifelse(kgC > canopy_thresh, kgC * later_growth, kgC)
    #kgC = ifelse(kgC >= K, kgC, kgC)
